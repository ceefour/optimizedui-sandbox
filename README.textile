...
that is partial + deferred asynchronous rendering, and unified templating for
...
3. Generates SEO-friendly HTML (all data is in there, not "hidden" inside JavaScript code)
4. DOM manipulation-friendly using JavaScript, jQuery, and Backbone
5. Embeds JSON data, so the Backbone is able to "make sense" what data is in the server-generated HTML
6. Built-in support for common AJAX requests that updates the DOM
7. Support asynchronous data push from server using Comet/WebSockets/SockJS and/or messaging infrastructure (AMQP, RabbitMQ, Stomp)
8. Support "simulated" asynchronous data push by using AJAX response as envelope
9. Supports hash-based (partial) navigation/bookmarkable fragment URIs
10. Both AJAX request/response and push messages support data payload, UI payload (probably declarative), or code payload
...
7. Spring MVC
8. Sling
9. Vaadin (OSGi support)
10. Oobium
11. OpenXava
12. ZK
13. Scout
14. RAP?
15. Custom framework on top of e4
16. Grails - most likely out
