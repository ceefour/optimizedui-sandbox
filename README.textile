...
that is partial + deferred asynchronous rendering.
...
2. Generates SEO-friendly HTML (all data is in there, not "hidden" inside JavaScript code)
3. DOM manipulation-friendly using JavaScript, jQuery, and Backbone
4. Embeds JSON data, so the Backbone is able to "make sense" what data is in the server-generated HTML
5. Built-in support for common AJAX requests that updates the DOM
6. Support asynchronous data push from server using Comet/WebSockets/SockJS and/or messaging infrastructure (AMQP, RabbitMQ, Stomp)
7. Support "simulated" asynchronous data push by using AJAX response as envelope
8. Supports hash-based (partial) navigation/bookmarkable fragment URIs
9. Both AJAX request/response and push messages support data payload, UI payload (probably declarative), or code payload
